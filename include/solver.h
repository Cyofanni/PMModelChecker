...
private:
	...
	void
	comp_symb_prog_meas(const Position &p, ExpNode *formula,
                                 const vector<int> &delta, const vector< vector< vector<int> > > &prog_meas_matr, vector<int> &pm_value);

	bool
	is_top(const vector<int> &pm_value) const;  //return 'true' if a given R(b)(i) has the Top value ('star')
...
public:
	...
	solve_system_worklist();

	vector< vector < vector<int> > >
	solve_system_chaotic_iteration();

	//lexicographic (<) order between vectors
	static bool
	lex_vectors(vector<int> a, vector<int> b, unsigned int from);

	static void
	print_pm_matrix(vector< vector < vector<int> > > pm_matrix, int bs, int ss);

	static void
	pretty_print_pm_matrix(vector< vector < vector<int> > > pm_matrix, int bs, int ss,
