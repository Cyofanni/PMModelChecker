...
string
ExpNode::get_node_name() const
{
    return node_name;
}

void
ExpNode::set_node_name(string new_name)
{
	node_name = new_name;
}
...
void
ExpNode::edit_index(ExpNode *e, string index)
{
	if (e)
	{
		//if node is an atom
 ...
 */
ExpNode*
ExpNode::edit_atoms(ExpNode *e, const vector<ExpNode*> &moves)
{
	if (e)
	{
		if (e->get_node_name()[0] == '[')
...
//only for moves
ExpNode*
ExpNode::deep_clone(ExpNode *src)
{
	if (src)
	{
		//if it is a leaf
...
void
ExpNode::get_leaves(ExpNode *&e, vector<ExpNode*> &leaves_array)
{
	if (e)
	{
...
void
ExpNode::graft_subtree(ExpNode *&e, ExpNode *where, ExpNode *new_subtree)
{
	if (e)
	{
		if (e == where)
...
void
ExpNode::print_equation_infix(ExpNode *e, FILE *handle)
{
	if (!e)
	{
		return;
...
int
ExpNode::get_max_index_in_equation(ExpNode *e)
{
	int max_index = -1;
...
string
Equation::get_id() const{
    return id;
}

string
Equation::get_eq_type() const
{
    return eq_type;
}

ExpNode*
Equation::get_exp_node() const
{
    return exp;
}
...
vector<Equation*>*
EqSystem::get_equations() const{
	return equations;
}

void
EqSystem::add_equation(Equation *eq)
{
...
vector<Equation*>
EqSystem::mucalculus_normalizer_system(vector<Equation*> system)
{
	vector<Equation*> res;
 ...
 */
int
Equation::mucalculus_normalizer_equation(ExpNode *eq_rhs, string left_id, string eq_type, int max_index, vector<Equation*> &equations)
{
	if (eq_rhs)
	{
