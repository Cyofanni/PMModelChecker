...
void
InputChecker::find_used_operators_equation(ExpNode *rhs, set<string> &operators) const
{
	if (rhs)
	{
...
vector<string>
InputChecker::get_phi_identifiers() const
{
...
set<string>
InputChecker::find_used_operators_system() const
{
...
int InputChecker::find_max_left_index() const
{
	int max_index = -1;
	for (int i = 0; i < eq_mv->size(); i++)
	{
		...
		if (curr_id[0] == VAR)
		{
			...
			if (var_index_int > max_index)
			{
				max_index = var_index_int;
			}
		}
	}
...
vector<string>
InputChecker::get_missing_moves()
{
...
int
InputChecker::find_out_of_bound_index() const
{
	int max_left_index = this->find_max_left_index();
