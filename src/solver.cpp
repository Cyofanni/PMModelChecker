...
void Solver::comp_symb_prog_meas(const Position &p, ExpNode *formula,
	...
	if (!(formula->left) and !(formula->right)){
		...
		if (atom != "true" and atom != "false"){  //if it is an atom like [b_0,1] etc...
		...
		else if (atom == "true"){  //if it is a 'true' 'atom'
			for (int i = 0; i < system_size; i++){
				pm_value[i] = 0;
			}
		...
		else if (atom == "false"){  //if it is a 'true' 'atom'
			for (int i = 0; i < system_size; i++){
				pm_value[i] = INT_MAX;
			}
	...
	//recursive cases
		...
		if (formula->get_node_name() == "or"){	//it's a minimum
		...
		else if (formula->get_node_name() == "and"){  //it's a maximum
			if (lex_vectors(pm_value, pm_value_1, 0)){
				pm_value = pm_value_1;
			}
			else{
...
bool Solver::is_top(const vector<int> &pm_value) const{
	bool istop = false;
...
vector< vector < vector<int> > > Solver::solve_system_worklist(){
	...
	for (int i = 0; i < basis_size; i++){	//loop on basis elements, +1 because empty set is added
			...
			if (y != prog_meas_matr[j][k]){
				...
				//else{
					...
					if (iter != depgraph.end()){
						for (int i = 0; i < iter->second.size(); i++){   //loop on dependencies of (b_j, k)
...
vector< vector < vector<int> > > Solver::solve_system_chaotic_iteration(){
	vector< vector < vector<int> > > prog_meas_matr(basis_size, vector<vector<int> >(system_size, vector<int>(system_size)));
...
bool Solver::lex_vectors(vector<int> a, vector<int> b, unsigned int from){
...
void Solver::print_pm_matrix(vector< vector < vector<int> > > pm_matrix, int bs, int ss){
